...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRolelat24"
...
resource "aws_ecs_service" "evolutionapi" {
  name            = "evolutionapi-servicelat24"
...
resource "aws_security_group" "redis_sglat24" {
  name        = "redis_sglat24"
...
resource "aws_vpc_endpoint" "s3lat24" {
  vpc_id            = data.aws_vpc.main.id
...
resource "aws_elasticache_cluster" "external" {
  ...
  security_group_ids   = [aws_security_group.redis_sglat24.id]
}

resource "aws_elasticache_subnet_group" "redis_subnets" {
  name       = "redis-subnet-grouplat24"
  subnet_ids = data.aws_subnets.private.ids
}
...
# Lambda: trigger-apilat24
resource "aws_lambda_function" "trigger_api" {
  function_name = "trigger-apilat24"
