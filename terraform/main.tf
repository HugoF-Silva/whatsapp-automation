...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRolelat25"
...
resource "aws_ecs_service" "evolutionapi" {
  name            = "evolutionapi-servicelat25"
...
resource "aws_security_group" "redis_sglat25" {
  name        = "redis_sglat25"
...
resource "aws_vpc_endpoint" "s3lat25" {
  vpc_id            = data.aws_vpc.main.id
...
resource "aws_elasticache_cluster" "external" {
  ...
  security_group_ids   = [aws_security_group.redis_sglat25.id]
}

resource "aws_elasticache_subnet_group" "redis_subnets" {
  name       = "redis-subnet-grouplat25"
  subnet_ids = data.aws_subnets.private.ids
}
...
# Lambda: trigger-apilat25
resource "aws_lambda_function" "trigger_api" {
  function_name = "trigger-apilat25"
