...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRolelat27"
...
resource "aws_ecs_service" "evolutionapi" {
  name            = "evolutionapi-servicelat27"
...
resource "aws_security_group" "redis_sglat27" {
  name        = "redis_sglat27"
...
resource "aws_elasticache_cluster" "external" {
  ...
  security_group_ids   = [aws_security_group.redis_sglat27.id]
}

resource "aws_elasticache_subnet_group" "redis_subnets" {
  name       = "redis-subnet-grouplat27"
  subnet_ids = data.aws_subnets.private.ids
}
...
# Lambda: trigger-apilat27
resource "aws_lambda_function" "trigger_api" {
  function_name = "trigger-apilat27"
