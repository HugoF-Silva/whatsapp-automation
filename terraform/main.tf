...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRolelat16"
...
resource "aws_ecs_service" "evolutionapi" {
  ...
  network_configuration {
    subnets         = data.aws_subnets.private.ids
    security_groups = [aws_security_group.redis_sglat16.id]
  }
  load_balancer {
    target_group_arn = aws_lb_target_group.evolutionapi.arn
...
resource "aws_security_group" "redis_sglat16" {
  name        = "redis_sglat16"
...
resource "aws_elasticache_cluster" "external" {
  ...
  security_group_ids   = [aws_security_group.redis_sglat16.id]
}

resource "aws_elasticache_subnet_group" "redis_subnets" {
  name       = "redis-subnet-grouplat16"
...
# Lambda: trigger-apilat16
resource "aws_lambda_function" "trigger_api" {
  function_name = "trigger-apilat16"
